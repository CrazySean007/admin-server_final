    ...
    .then(user => {
      if (user) { // 登陆成功
        ...
        if (user.role_id) {
          ...
          RoleModel.findOne({name: user.role_id})
            .then(role => {
              user._doc.role = role
...
router.post('/manage/user/delete', (req, res) => {
  ...
  UserModel.deleteOne({_id: userId})
    .then((doc) => {
      console.log("deletion user with ID: "+userId)
      res.send({status: 0})
    })
})
...
router.get('/manage/category/info', (req, res) => {
  ...
  CategoryModel.findOne({_id: categoryId})
    ...
    .catch(error => {
      ...
      res.send({status: 1, msg: 'get category info failed, please try again!'})
    })
})
...
router.get('/manage/product/search', (req, res) => {
  ...
  ProductModel.find(contition)
    ...
    .catch(error => {
      console.error('search product list failed!', error)
      res.send({status: 1, msg: 'search product list failed, please try again!'})
    })
})
...
router.post('/manage/role/update', (req, res) => {
  ...
  RoleModel.findOneAndUpdate({_id: role._id}, role)
    ...
    .catch(error => {
      ...
      res.send({status: 1, msg: 'update role failed, please try again!'})
    })
})
